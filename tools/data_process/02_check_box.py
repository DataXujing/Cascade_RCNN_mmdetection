    ...
    for anna_file in files:
        ...
        for obj in root.findall('object'):
            ...
            if (float(bbox.find('ymin').text) >= float(bbox.find('ymax').text)) or (float(bbox.find('xmin').text) >= float(bbox.find('xmax').text)):
                ...
                i += 1
                try:
                    shutil.move(path+"/"+anna_file,"./data/coco/error/"+anna_file)
                    shutil.move("./data/coco/train/JPEGImages/"+anna_file.split(".")[0]+".jpg","./data/coco/error/"+anna_file.split(".")[0]+".jpg")
                except:
                    pass
...
if __name__ == "__main__":
    check_box("./data/coco/train/annotations")